...
const int MIN_SECTOR_COUNTs = 3;
const int MIN_STACK_COUNTs = 2;
...
public:
    ...
    void set(float radius, int sectors, int stacks, glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shiny)
    {
        ...
        if (sectors < MIN_SECTOR_COUNTs)
            this->sectorCount = MIN_SECTOR_COUNTs;
        this->stackCount = stacks;
        if (stacks < MIN_STACK_COUNTs)
            this->stackCount = MIN_STACK_COUNTs;
        this->ambient = amb;
        this->diffuse = diff;
        this->specular = spec;
    ...
    void drawSphere(Shader& lightingShader, glm::mat4 model = glm::mat4(1.0f), float r = 1.0f, float g = 1.0f, float b = 1.0f) const      // draw surface
    {
        lightingShader.use();
